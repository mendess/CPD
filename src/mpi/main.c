...
int main(int argc, char** argv) {
    ...
    if (me == 0) {
        ...
        size_t send_size_t[] = {
            matrices.num_iterations,
            matrices.a_prime._total_items,
            matrices.a_prime.n_rows,
            matrices.a_prime.n_cols,
            matrices.a_prime_transpose.current_items,
            matrices.a_prime_transpose.n_rows,
            matrices.a_prime_transpose.n_cols,
            matrices.l.rows,
            matrices.l.columns,
            matrices.r.rows,
            matrices.r.columns};
        for (int i = 1; i < nprocs; i++) {
            ...
            // matrices.a_prime.row_pos[end + 1] - 1 -
            // matrices.a_prime.row_pos[start]);
            mpi_send_items(
                matrices.a_prime.items,
                matrices.a_prime.current_items,
                i,
                0,
                MPI_COMM_WORLD);
            mpi_send_items(
                matrices.a_prime_transpose.items,
                matrices.a_prime_transpose.current_items,
                i,
                0,
                MPI_COMM_WORLD);
        ...
        */
        ...
        matrices.a_prime.current_items = send_size_t[1];
        matrices.a_prime.n_rows = send_size_t[2];
        matrices.a_prime.n_cols = send_size_t[3];
        matrices.a_prime_transpose.current_items = send_size_t[4];
        ...
        matrices.a_prime.items =
            malloc(sizeof(Item) * matrices.a_prime.current_items);
        matrices.a_prime_transpose.items =
            malloc(sizeof(Item) * matrices.a_prime_transpose.current_items);
    ...
    /* CompactMatrix* mm[2] = {&matrices.a_prime, &matrices.a_prime_transpose};
     */
    /* char c[2] = {'a', 't'}; */
